...
namespace Opc.Ua.Server
{
    ...
    public class CustomNodeManager2 : INodeManager2, INodeIdFactory, IDisposable
    {
        ...
        public virtual void LookupAliasesForNodeIds(ServerSystemContext context, NodeId[] nodeIds, string[] aliases)
        {
            Dictionary<NodeId, NodeState> cache = new Dictionary<NodeId, NodeState>();

            for (int ii = 0; ii < nodeIds.Length; ii++)
            {
                NodeHandle handle = GetManagerHandle(context, nodeIds[ii], cache);

                if (handle != null)
                {
                    aliases[ii] = LookupAliasForNodeId(context, handle, cache);
                }
            }
        }
        ...
        /// </summary>
        protected virtual string LookupAliasForNodeId(
            ServerSystemContext context,
            NodeHandle handle,
            IDictionary<NodeId, NodeState> cache)
        {
            StringBuilder alias = new StringBuilder();
            ConstructAlias(context, null, handle, alias, cache);
        ...
        /// </summary>
        protected virtual void ConstructAlias(
            ServerSystemContext context,
            string prefix,
            NodeHandle handle,
            StringBuilder alias,
            IDictionary<NodeId, NodeState> cache)
        {
            // append prefix used to identify the node manager.
            alias.Append(AliasRoot);
            alias.Append('/');

            // append another prefix used to a subcomponent defined by the node manager.
            ...
            // check if node is part of a tightly coupled instance inhierarchy
            NodeId nodeId = handle.NodeId;
            NodeState root = handle.Node;

            // build the hierarchy.
            Stack<NodeState> hierarchy = null;

            if (root != null)
            {
                hierarchy = new Stack<NodeState>();

                for (NodeState node = handle.Node; node != null; )
                {
                    root = node;
                    BaseInstanceState instance = node as BaseInstanceState;
                ...
                if (root != null && !NodeId.IsNull(root.NodeId))
                {
                    nodeId = root.NodeId;
                }
            }

            // add the node id type.
            alias.Append((int)nodeId.IdType);

            // add a fixed index for the namespace uri 
            // i.e. assume that the list of namespace uris assigned to a node manager are always assigned in the same order.
            for (int ii = 0; ii < NamespaceIndexes.Length; ii++)
            {
                if (handle.NodeId.NamespaceIndex == NamespaceIndexes[ii])
                {
                    alias.Append(ii);
                    break;
                }
            ...
            {
                alias.Append(Utils.ToHexString((byte[])nodeId.Identifier));
            }
            else
            {
                alias.Append(nodeId.Identifier);
            }

            alias.Append('#');

            // nothing more to do if no node object available.
            if (root == null)
            {
                return;
            }

            // append the browse name for the root if not using string ids.
            // this assumes that string ids already contain a human readable name for the root.
            bool nameEncoded = false;

            if (root.NodeId.IdType != IdType.String)
            {
                nameEncoded = true;
                EncodeName(root, alias);
            }

            // append hierarchy element names.
            while (hierarchy.Count > 0)
            {
                NodeState node = hierarchy.Pop();
                ...
                EncodeName(node, alias);
            }
        }

        /// <summary>
        /// Encodes the name for the node.
        /// </summary>
        private void EncodeName(NodeState node, StringBuilder alias)
        {
            if (node != null)
            {
                if (!String.IsNullOrEmpty(node.SymbolicName))
                {
                    EncodeName(node.SymbolicName, alias);
                }
                else if (node.BrowseName != null)
                {
                    EncodeName(node.BrowseName.Name, alias);
                }
            }
        }

        /// <summary>
        /// Encodes the name in the alias.
        /// </summary>
        private void EncodeName(string name, StringBuilder alias)
        {
            if (String.IsNullOrEmpty(name) || alias == null)
            {
                return;
            }
        ...
        /// <summary>
        /// Decodes the symbolic path.
        /// </summary>
        private int DecodePath(string alias, Stack<string> symbolicPath)
        {
            if (String.IsNullOrEmpty(alias) || symbolicPath == null)
            {
                return -1;
            }

            StringBuilder buffer = new StringBuilder(alias.Length);

            for (int ii = alias.Length - 1; ii >= 0; ii--)
            {
                char ch = alias[ii];
                ...
                if (ch == '#' || ch == '%' || ch == '/')
                {
                    ...
                    if (ch == '/')
                    {
                        symbolicPath.Push(buffer.ToString());
                        buffer.Length = 0;
                        continue;
                    }
        ...
        /// <param name="aliases">The aliases.</param>
        /// <param name="nodeIds">The node ids.</param>
        public virtual void LookupNodeIdsForAliases(ServerSystemContext context, string[] aliases, NodeId[] nodeIds)
        {
            Dictionary<NodeId, NodeState> cache = new Dictionary<NodeId, NodeState>();

            for (int ii = 0; ii < aliases.Length; ii++)
            {
                string alias = aliases[ii];

                if (alias != null && alias.StartsWith(m_aliasRoot))
                {
                    try
                    {
                        nodeIds[ii] = LookupNodeIdForAlias(context, aliases[ii], cache);
                    }
                    catch (Exception)
                    {
                        nodeIds[ii] = null;
        ...
        /// </summary>
        protected virtual NodeId LookupNodeIdForAlias(
            ServerSystemContext context,
            string alias,
            IDictionary<NodeId, NodeState> cache)
        {
            return ParseAlias(context, null, alias, cache);
        }

        /// <summary>
        /// Parses the alias.
        /// </summary>
        protected virtual NodeId ParseAlias(
            ServerSystemContext context,
            string prefix,
            string alias,
            IDictionary<NodeId, NodeState> cache)
        {
            ...
            // extract root assigned to node manager.
            if (AliasRoot != null)
            {
                if (!alias.StartsWith(AliasRoot))
                {
                    return null;
                }

                index = AliasRoot.Length;
            }

            // check for trailing seperator.
            if (alias.Length <= index || alias[index] != '/')
            {
                return null;
            }
            ...
            // extract additional prefix assigned by the node manager.
            if (!String.IsNullOrEmpty(prefix))
            {
            ...
            // extract id type.
            IdType idType = (IdType)((int)alias[0] - '0');

            // extract namespace index.
            index = 1;
            int namespaceIndex = 0;

            while (index < 5 && index < alias.Length && alias[index] != ':')
            {
                namespaceIndex *= 10;
                namespaceIndex += ((int)alias[index++] - '0');
            }
            ...
            if (namespaceIndex >= NamespaceIndexes.Length)
            {
                return null;
            }
            ...
            // extract path.
            Stack<string> symbolicPath = new Stack<string>();
            index = DecodePath(alias, symbolicPath);
            ...
            try
            {
                ...
                {
                    rootId = new NodeId(System.Convert.ToUInt32(alias), NamespaceIndexes[namespaceIndex]);
                }

                else if (idType == IdType.String)
                {
                    rootId = new NodeId(alias, NamespaceIndexes[namespaceIndex]);
                }

                else if (idType == IdType.Guid)
                {
                    rootId = new NodeId(new Guid(alias), NamespaceIndexes[namespaceIndex]);
                }

                else if (idType == IdType.Opaque)
                {
                    rootId = new NodeId(Utils.FromHexString(alias), NamespaceIndexes[namespaceIndex]);
                }
            }
            catch (Exception)
            {
            ...
            // validate node.
            NodeHandle handle = GetManagerHandle(context, rootId, cache);
            ...
            if (!handle.Validated)
            {
                root = ValidateNode(context, handle, cache);
            }
            ...
            if (symbolicPath.Count == 0)
            {
                return root.NodeId;
            }

            // find tightly coupled child node.
            // ignore top level browse name since node id is value.
            if (idType != IdType.String)
            {
                symbolicPath.Pop();
            }

            // follow path to find target child.
            NodeState node = root;
            List<BaseInstanceState> children = new List<BaseInstanceState>();

            while (symbolicPath.Count > 0)
            {
                string name = symbolicPath.Pop();
                ...
                for (int ii = 0; ii < children.Count; ii++)
                {
                    if (name == children[ii].SymbolicName)
                    {
                        found = true;
                        node = children[ii];
                        break;
                ...
                if (!found)
                {
                    ...
                    for (int ii = 0; ii < children.Count; ii++)
                    {
                        if (children[ii].BrowseName != null)
                        {
                            if (name == children[ii].BrowseName.Name)
                            {
                                found = true;
                                node = children[ii];
                                break;
