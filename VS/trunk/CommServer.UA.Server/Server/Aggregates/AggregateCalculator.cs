...
namespace Opc.Ua.Server
{
    ...
    public class AggregateCalculator : IAggregateCalculator
    {
        ...
        public AggregateCalculator()
        {
            ...
            Initialize(ObjectIds.AggregateFunction_Interpolative, DateTime.UtcNow, DateTime.MaxValue, 1000, false, configuration);
        }
        ...
        public DataValue GetProcessedValue(bool returnPartial)
        {
            ...
            DateTime earlyTime = (TimeFlowsBackward)?CurrentSlice.EndTime:CurrentSlice.StartTime;
            DateTime lateTime = (TimeFlowsBackward)?CurrentSlice.StartTime:CurrentSlice.EndTime;

            if (CompareTimestamps(earlyTime, m_values.First) < 0 || CompareTimestamps(lateTime, m_values.Last) > 0)
            {
        ...
        protected bool UpdateSlice(TimeSlice slice)
        {
            ...
            {
                // no need to continue if values already processed.
                if (TimeFlowsBackward && Object.ReferenceEquals(ii, slice.LastProcessedValue))
                {
                    break;
                }
        ...
        {
            if (AggregateId == ObjectIds.AggregateFunction_TimeAverage)
            {
                return TimeAverage(slice);
            }

            if (AggregateId == ObjectIds.AggregateFunction_TimeAverage2)
            {
                return TimeAverageBounds(slice);
            }

            if (AggregateId == ObjectIds.AggregateFunction_Start2)
            {
                return StartBounds(slice);
            }

            if (AggregateId == ObjectIds.AggregateFunction_End2)
            {
                return EndBounds(slice);
            }
            ...
            {
                return CountOrTotal(slice, false);
            }

            if (AggregateId == ObjectIds.AggregateFunction_Total)
            {
                return CountOrTotal(slice, true);
            }
        ...
        /// Calculate the interpolate aggregate for the timeslice.
        /// </summary>
        protected DataValue Average(TimeSlice slice)
        {
            ...
            {
                // do not include interpolated bounds in the calculation.
                if ((region.DataPoint.StatusCode.AggregateBits & AggregateBits.Interpolated) != 0)
                {
                    continue;
                }

                if (!IsGood(region.DataPoint))
                {
                    statusCode = StatusCodes.UncertainSubNormal;
                    continue;
                }

                count++;
                total += region.StartValue;
            }
            ...
            value.WrappedValue = new Variant(average, TypeInfo.Scalars.Double);
            value.SourceTimestamp = slice.StartTime;
            value.ServerTimestamp = slice.StartTime;
        ...
        /// Calculate the interpolate aggregate for the timeslice.
        /// </summary>
        protected DataValue CountOrTotal(TimeSlice slice, bool returnTotal)
        {
            StatusCode statusCode = StatusCodes.Good;
            statusCode = statusCode.SetAggregateBits(AggregateBits.Calculated);

            DataValue value = new DataValue();
            value.WrappedValue = (returnTotal)?new Variant((double)0, TypeInfo.Scalars.Double):new Variant((int)0, TypeInfo.Scalars.Int32);
            value.SourceTimestamp = slice.StartTime;
            value.ServerTimestamp = slice.StartTime;
            value.StatusCode = statusCode;
            ...
            foreach (SubRegion region in regions)
            {
                ...
                if (!IsGood(region.DataPoint))
                {
                    if (returnTotal)
                    {
                        statusCode = statusCode.SetCodeBits(StatusCodes.UncertainSubNormal);
                    }
                ...
                if (returnTotal)
                {
                    if (StatusCode.IsNotGood(region.DataPoint.StatusCode))
                    {
                        statusCode = statusCode.SetCodeBits(StatusCodes.UncertainSubNormal);
                    }
                }
            ...
            if (returnTotal)
            {
                value.WrappedValue = new Variant(total, TypeInfo.Scalars.Double);
            }
            else
            {
        ...
        /// Calculate the interpolate aggregate for the timeslice.
        /// </summary>
        protected DataValue TimeAverage(TimeSlice slice)
        {
            // get the values in the slice.
            List<DataValue> values = GetValuesWithInterpolatedBounds(slice);
            ...
            // get the regions in the slice.
            List<SubRegion> regions = GetRegionsInValueSet(values, true, Stepped);
            ...
            // compute the status first - nothing to do if bad.
            StatusCode statusCode = GetStatusCodeForSlice(regions);

            if (StatusCode.IsBad(statusCode))
            {
                return new DataValue(Variant.Null, statusCode, slice.StartTime, slice.StartTime);
            }
            ...
            foreach (SubRegion region in regions)
            {
                ...
                // use the first non-null value as the reference for converting back to the original type.
                if (dataPoint == null || dataPoint.WrappedValue == Variant.Null)
                {
                    dataPoint = region.DataPoint;
                }

                if (Stepped)
                {
                    total += region.Duration*region.StartValue;
                }
                else
                {
                    total += region.Duration*(region.StartValue + region.EndValue)/2;
                }
            }

            double average = total/totalDuration;

            // create processed value.
            DataValue value = new DataValue();

            value.Value = CastToOriginalType(average, dataPoint);
            value.SourceTimestamp = slice.StartTime;
            value.ServerTimestamp = slice.StartTime;
            value.StatusCode = statusCode;

            if (slice.OutOfDataRange && StatusCode.IsGood(value.StatusCode))
            {
                value.StatusCode = statusCode.SetCodeBits(StatusCodes.UncertainSubNormal);
            }
        ...
        /// Calculate the interpolate aggregate for the timeslice.
        /// </summary>
        protected DataValue TimeAverageBounds(TimeSlice slice)
        {
            // get the values in the slice.
            List<DataValue> values = GetValuesWithSimpleBounds(slice);
        ...
        /// <summary>
        /// Get the simple start bound for the time slice.
        /// </summary>
        protected DataValue StartBounds(TimeSlice slice)
        {
            return GetSimpleBound(slice.StartTime, slice);
        }

        /// <summary>
        /// Get the simple end bound for the time slice.
        /// </summary>
        protected DataValue EndBounds(TimeSlice slice)
        {
            DataValue dataValue = GetSimpleBound(slice.EndTime, slice);
            dataValue.ServerTimestamp = slice.StartTime;
            dataValue.SourceTimestamp = slice.StartTime;
            return dataValue;
        }
        ...
        protected DataValue Interpolate(DateTime timestamp, TimeSlice reference)
        {
            ...
            if (!Stepped)
            {
                ...
                if (slice.EarlyBound != null && slice.LateBound != null)
                {
                    ...
                    {
                        dataValue.StatusCode = dataValue.StatusCode.SetCodeBits(StatusCodes.UncertainSubNormal);
                    }
        ...
        /// </summary>
        protected DataValue SteppedInterpolate(DateTime timestamp, DataValue earlyBound)
        {
            ...
            {
                dataValue.StatusCode = StatusCodes.UncertainSubNormal;
            }
        ...
        /// Calculate the value at the timestamp using slopped interpolation.
        /// </summary>
        protected DataValue SlopedInterpolate(DateTime timestamp, DataValue earlyBound, DataValue lateBound)
        {
            try
            {
                // convert to doubles.
        ...
        /// </summary>
        protected double CastToDouble(DataValue value)
        {
        ...
        /// </summary>
        protected Variant CastToOriginalType(double value, DataValue original)
        {
        ...
        protected DataValue GetSimpleBound(DateTime timestamp, TimeSlice slice)
        {
            ...
            {
                value.StatusCode = StatusCodes.UncertainSubNormal;
        ...
        protected List<DataValue> GetValuesWithSimpleBounds(TimeSlice slice)
        {
            ...
            if (TimeFlowsBackward)
            {
                if (CompareTimestamps(slice.EndTime, m_values.Last) > 0  || CompareTimestamps(slice.StartTime, m_values.First) < 0)
                {
                    return null;
                }
            }
        ...
        protected List<DataValue> GetValuesWithInterpolatedBounds(TimeSlice slice)
        {
            ...
            if (TimeFlowsBackward)
            {
                if (CompareTimestamps(slice.StartTime, m_values.First) < 0)
                {
                    return null;
                }
            }
        ...
        /// <returns></returns>
        protected StatusCode GetStatusCodeForSlice(List<SubRegion> regions)
        {
            // check for empty set.
            if (regions == null || regions.Count == 0)
            {
