...
namespace Opc.Ua.Server
{
    ...
    public class MasterNodeManager : IDisposable
    {
        ...
        /// </summary>
        public virtual void ImportNodeSet(NodeSet nodeSet)
        {
            try
            {         
                UpdateLock.Enter();
                ...
                CoreNodeManager.ImportNodeSet(nodeSet, externalReferences);
        ...
        /// </summary>
        private ServiceResult TranslateBrowsePath(
            OperationContext context,
        ...
        /// </summary>
        private ServiceResult FetchReferences(
            OperationContext                   context,
            bool                               assignContinuationPoint,
        ...
        public virtual void LookupAliasesForNodeIds(ServerSystemContext context, NodeId[] nodeIds, string[] aliases)
        {
            ...
            foreach (INodeManager nodeManager in m_nodeManagers)
            {
                try
                {
                    ...
                    {
                        nodeManager2.LookupAliasesForNodeIds(context, nodeIds, aliases);
                    }
                }
                catch (Exception e)
                {
                    Utils.Trace(e, "Error calling LookupAliasesForNodeIds on NodeManager.");
                }
            }

            // serialize an absolute node id.
            for (int ii = 0; ii < nodeIds.Length; ii++)
            {
                NodeId nodeId = nodeIds[ii];

                if (!NodeId.IsNull(nodeId) && String.IsNullOrEmpty(aliases[ii]))
                {
                    ...
                    {
                        string namespaceUri = Server.NamespaceUris.GetString(nodeIds[ii].NamespaceIndex);

                        if (!String.IsNullOrEmpty(namespaceUri))
                        {
                            buffer.Append(new ExpandedNodeId(nodeId, namespaceUri));
                        }
                    }
                    else
                    {
        ...
        public virtual void LookupNodeIdsForAliases(ServerSystemContext context, string[] aliases, NodeId[] nodeIds)
        {
            foreach (INodeManager nodeManager in m_nodeManagers)
            {
                try
                {
                    ...
                    if (nodeManager2 != null)
                    {
                        nodeManager2.LookupNodeIdsForAliases(context, aliases, nodeIds);
                    }
                }
                catch (Exception e)
                {
                    Utils.Trace(e, "Error calling LookupNodeIdsForAliases on NodeManager.");
                }
            }

            // deserialize an absolute node id.
            for (int ii = 0; ii < nodeIds.Length; ii++)
            {
                if (NodeId.IsNull(nodeIds[ii]) && aliases[ii] != null && aliases[ii].StartsWith("ID/"))
                {
                    try
                    {
                        ExpandedNodeId nodeId = ExpandedNodeId.Parse(aliases[ii].Substring(3));
                        nodeIds[ii] = ExpandedNodeId.ToNodeId(nodeId, Server.NamespaceUris);
                    }
                    catch
                    {
                        nodeIds[ii] = null;
