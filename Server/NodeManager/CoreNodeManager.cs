...
namespace Opc.Ua.Server
{
    ...
    {        
            ...
            ushort                   dynamicNamespaceIndex)
        {
            ...
            m_modelChanges                   = new Dictionary<ILocalNode,ModelChangeStructureVerbMask>();
            m_minModelChangeInterval         = 5000;
        ...
        /// </summary>
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2213:DisposableFieldsShouldBeDisposed", MessageId = "m_samplingGroupManager"), System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2213:DisposableFieldsShouldBeDisposed", MessageId = "m_modelChangeTimer")]
        ...
        {  
            if (disposing)
            {
                ...
                Utils.SilentDispose(m_modelChangeTimer);
            }
        }
        #endregion
        ...
        /// </summary>
        public bool SuppressModelChangeEvents
        {
            get
            {
                try
                {
                    m_lock.Enter();
                    return m_suppressModelChangeEvents;
                }
                finally
                {
                    m_lock.Exit();
            ...
            set
            {
                ...
                {
                    m_lock.Enter();
                    m_suppressModelChangeEvents = value;
                }
                finally
                {
                    m_lock.Exit();
        ...
        internal void ImportNodes(ISystemContext context, IEnumerable<NodeState> predefinedNodes, bool isInternal)
        {
            ...
            {               
                try
                {
                    Server.CoreNodeManager.SuppressModelChangeEvents = true;
                ...
                {
                    Server.CoreNodeManager.SuppressModelChangeEvents = false;
                }
            }
        }
        ...
        /// </summary>
        public void ImportNodeSet(NodeSet nodeSet, IDictionary<NodeId,IList<IReference>> externalReferences)
        {
            try
            {
                m_lock.Enter();

                List<Node> nodes = m_nodes.Import(nodeSet, externalReferences);

                // need to make sure the diagnostic node manager knows about any references to built-in nodes.
                for (int ii = 0; ii < nodes.Count; ii++)
                {
                    ILocalNode node = nodes[ii];

                    foreach (IReference reference in node.References)
                    {
                        ILocalNode source = GetLocalNode(reference.TargetId);
                        ...
                        if (source.NodeId.NamespaceIndex != 0)
                        {
                            continue;
                        }

                        if (reference.ReferenceTypeId.IdType != IdType.Numeric)
                        {
                            continue;
                        }
                        ...
                        switch ((uint)reference.ReferenceTypeId.Identifier)
                        {
                            ...
                            default:
                            {
                                AddReferenceToLocalNode(source, reference.ReferenceTypeId, !reference.IsInverse, node.NodeId, false);
                                break;
                            }
                        }
                    }
                ...
                // build table of nodes to process.
                Dictionary<NodeId,bool> processedNodes = new Dictionary<NodeId,bool>();

                foreach (Node node in nodes)
                {
                    processedNodes[node.NodeId] = false; 
                }

                // apply modelling rules to all instance nodes.
                foreach (Node node in nodes)
                {
                    ImportNode(node, processedNodes);
                }
            }
            finally
            {
        ...
        /// <summary>
        /// Ensures that the modelling rules have been applied to an imported node.
        /// </summary>
        /// <remarks>
        /// Caller must hold lock on NodeManager.
        /// </remarks>
        private void ImportNode(Node node, Dictionary<NodeId,bool> processedNodes)
        {
            // ignore nodes that are not in the list of nodes being imported.
            bool processed = false;

            if (!processedNodes.TryGetValue(node.NodeId, out processed))
            {
                return;
            }

            // ignore nodes which have already been processed.
            if (processed)
            {
                return;
            }    

            // ignore nodes without a type definition.
            ExpandedNodeId typeDefinitionId = node.TypeDefinitionId;
            ...
            processedNodes[node.NodeId] = true;

            // ensure that the type definition has been processed.
            if (processedNodes.TryGetValue(typeDefinition.NodeId, out processed))
            {
                if (!processed)
                {
                    processedNodes[typeDefinition.NodeId] = true;

                    IList<IReference> references = typeDefinition.References.Find(ReferenceTypeIds.Aggregates, false, true, m_server.TypeTree);

                    foreach (IReference reference in references)
                    {
                        Node child = GetLocalNode(reference.TargetId) as Node;

                        if (child != null)
                        {
                            ImportNode(child, processedNodes);
                        }
                    }          
                }    
            }
            ...
            IList<ILocalNode> nodesToAdd = factory.ApplyModellingRules(node, typeDefinition.NodeId, ref m_lastId, 1);
        ...
        public void CreateAddressSpace(IDictionary<NodeId,IList<IReference>> externalReferences)
        {
            try
            {
                ...
                bool suppressModelChangeEvents = m_suppressModelChangeEvents;
                
                try
                {
                    // set this prevents model change events from being reported during the import.
                    m_suppressModelChangeEvents = true;
                    ...
                    // Stream istrm = Assembly.GetExecutingAssembly().GetManifestResourceStream("Opc.Ua.Server.AddressSpace.Model.Opc.Ua.NodeSet.xml");
                    // ImportNodeSet(NodeSet.Read(istrm), externalReferences);
                }
                finally
                {
                    m_suppressModelChangeEvents = suppressModelChangeEvents;
                }
            }
            finally
            {
            ...
            ushort     namespaceIndex)
        {
            ...
            foreach (ILocalNode type in typeDefinitions)
            {
                foreach (IReference reference in type.References)
                {
                    ...
                    // do not copy the model parent reference for shared nodes.
            ...
            bool   bidirectional)
        {
            try
            {
                ...
                {              
                    ...
                    QueueModelChange(target, ModelChangeStructureVerbMask.ReferenceAdded);
                }
                
                // add reference from source to target.                
                AddReferenceToLocalNode(source, referenceTypeId, isInverse, targetId, false);
                QueueModelChange(source, ModelChangeStructureVerbMask.ReferenceAdded);
            ...
            bool           deleteBidirectional)
        {
            ...
            try
            {
                ...
                QueueModelChange(source, ModelChangeStructureVerbMask.ReferenceDeleted);
                ...
                if (deleteBidirectional)
                {
                    ...
                    {
                        target.References.Remove(referenceTypeId, !isInverse, source.NodeId);
                        QueueModelChange(target, ModelChangeStructureVerbMask.ReferenceDeleted);
                    }
                }
        ...
        {            
            m_nodes.Attach(node);
            QueueModelChange(node, ModelChangeStructureVerbMask.NodeAdded);
        }

        /// <summary>
        /// Queues a model change.
        /// </summary>
        private void QueueModelChange(ILocalNode node, ModelChangeStructureVerbMask changeType)
        {
            try
            {
                m_lock.Enter();

                // do nothing if model change events are deactivated.
                if (m_suppressModelChangeEvents)
                {
                    return;
                }

                ModelChangeStructureVerbMask existingChanges;

                if (m_modelChanges.TryGetValue(node, out existingChanges))
                {
                    changeType |= existingChanges;
                }

                m_modelChanges[node] = changeType;

                if (m_modelChangeTimer == null)
                {
                    m_modelChangeTimer = new Timer(new TimerCallback(ReportModelChange), null, m_minModelChangeInterval, m_minModelChangeInterval);
                }
            }
            finally
            {
        ...
        /// <summary>
        /// Reports a model change event.
        /// </summary>
        private void ReportModelChange(object state)
        {
            try
            {
                m_lock.Enter();
                ...
                ModelChangeStructureDataTypeCollection changes = new ModelChangeStructureDataTypeCollection();

                foreach (KeyValuePair<ILocalNode,ModelChangeStructureVerbMask> current in m_modelChanges)
                {
                    ModelChangeStructureDataType change = new ModelChangeStructureDataType();

                    change.Affected     = current.Key.NodeId;
                    change.AffectedType = (NodeId)current.Key.TypeDefinitionId;
                    change.Verb         = (byte)current.Value;
                ...
                if (changes.Count == 0)
                {
                    if (m_modelChangeTimer != null)
                    {
                        m_modelChangeTimer.Dispose();
                        m_modelChangeTimer = null;
                    }
                ...
                GeneralModelChangeEventState e = new GeneralModelChangeEventState(null);
                ...
                e.SetChildValue(Server.DefaultSystemContext, BrowseNames.SourceNode, ObjectIds.Server, false);
                e.SetChildValue(Server.DefaultSystemContext, BrowseNames.SourceName, "AddressSpace", false);
                e.SetChildValue(Server.DefaultSystemContext, BrowseNames.Changes, changes.ToArray(), false);

                m_server.ReportEvent(Server.DefaultSystemContext, e);
            }
            catch (Exception e)
            {
                Utils.Trace(e, "Unexpected error reporting model change event."); 
            }
            finally
            {
                m_lock.Exit();
        ...
        private Dictionary<ILocalNode,ModelChangeStructureVerbMask> m_modelChanges;
        private Timer m_modelChangeTimer;
        private int m_minModelChangeInterval;
        private bool m_suppressModelChangeEvents;
